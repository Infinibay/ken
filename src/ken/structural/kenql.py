...
RELATIONS = frozenset('HAS_INITIALIZER FIELD_NAME INITIALIZES_FIELD ITERATED_CALL OPTIONAL_VALUE OPTIONAL_OR OPTIONAL_PRESENT OPTIONAL_FALLBACK BODY_VALUE NULL_TEST BRANCH_TRUE BRANCH_FALSE SYNTAX_NODE SYNTAX_PARENT HANDLER_OF ENCLOSING_LOOP IN_HANDLER IN_TRY_BODY CONTINUE_TARGET DERIVE_NAME INVOKES_RESULT_OF CONTAINER CALLEE_VALUE INDEX ITERATES_KEYS_CALLS ADVANCES_ITERATOR MEMBER_OF INSERTS_INTO INSERTED_VALUE LOADED_FROM ACQUIRES ACQUIRES_LOCK ALLOCATES_TYPE ARGUMENT ASSIGN ASSIGNED_FROM AWAIT BASE_NAME BINDS_TO BRANCH CALL CALLABLE CALLEE_NAME CALLS CAPTURES CFG_NEXT CLASS COLLECTION CONDITIONAL_DELEGATION CONFORMS_TO CONTEXT_ENTRY CREATES CREATES_CONTEXT CREATES_LOCK DECLARES DECORATED_BY DECORATOR DELEGATES_TO DELEGATES_TYPE ELEMENT_TYPE EMBEDS ENTITY EXPORT EXPORT_SYNTAX EXTENDS FLOWS_TO FUNCTION GUARDS_WRITE HAS_ HAS_ASYNC_SCOPE HAS_CALL HAS_FIELD HAS_HAZARD HAS_METHOD HAS_OPERATION HAS_PARAMETER HAZARD IMPLEMENTS IMPORT IMPORT_SYNTAX INITIALIZED_AS INSTANCE_OF INTERFACE IN_TYPE IS ITERATES_CALLS JOINS LOOKS_UP LOOP MATCH MAY_ MAY_BIND_TO MAY_CALLS MAY_TARGET MAY_TYPE MEMBER MODULE NATIVE NULL OPERATION OVERRIDES OWNED_BY OWNER PARAMETER PASSES_SELF_TO READS RECEIVER RELEASES RELEASES_LOCK RESOURCE_SCOPE RESULT RETURN RETURNS RETURNS_CALL RETURNS_LOOKUP RETURNS_NEW RETURNS_NEW_SELF RETURNS_SELF RETURNS_STORAGE RETURNS_VALUE SATISFIES SPAWNS_CONTEXT SPREAD STARTS_CONTEXT STORAGE STORES_VALUE SUBTYPE_OF TARGET THROW TRY TYPE TYPE_NAME TYPE_PARAMETER VALUE VALUE_FLOW VARIADIC WAITS_FOR WRITES WRITES_ELEMENT YIELD _PARSER _PARSER_TS _PARSER_TSX'.split())
...
RELATIONS |= frozenset({'TRUTH_TEST'})
